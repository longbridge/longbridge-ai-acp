...
impl RichTextFilter {
    ...
    fn push(&mut self, text: &str) -> Vec<FilteredContent> {
        ...
        const SVG_MARKER: &str = "```svg-inline";
        ...
        loop {
            ...
            let svg_open = self.buffer.find(SVG_MARKER);
            ...
            let selected = [
                ...
                svg_open.map(|open| (open, SVG_MARKER, 6_u8)),
            ...
            if kind == 6 {
                match RichContent::svg(
                    format!("{}:svg-{}", self.content_id_prefix, self.next_svg),
                    body.trim(),
                    "SVG diagram",
                ) {
                    Ok(svg) => {
                        self.next_svg += 1;
                        output.push(FilteredContent::Rich(svg));
                    }
                    Err(_) => output.push(FilteredContent::Text(
                        "SVG content was omitted because it could not be rendered safely."
                            .to_owned(),
                    )),
                }
                continue;
...
mod tests {
    ...
    #[test]
    fn svg_inline_becomes_safe_image_without_raw_markup_fallback() {
        let mut filter = RichTextFilter::new("session-1");
        assert!(filter
            .push("Before\n```svg-in")
            .iter()
        ...
        let FilteredContent::Rich(svg) = &output[0] else {
            panic!("expected SVG rich content");
        };
        assert_eq!(svg.kind, crate::RichContentKind::Svg);
        assert_eq!(svg.fallback, "SVG diagram");
        assert!(!svg.fallback.contains("<svg"));
        assert!(matches!(output.get(1), Some(FilteredContent::Text(text)) if text == "After"));
    }

    #[test]
    fn unsafe_svg_inline_is_omitted_instead_of_echoed() {
        let mut filter = RichTextFilter::new("session-1");
        let output = filter.push("```svg-inline\n<svg onload=\"alert(1)\"></svg>\n```");
        assert!(matches!(
            output.first(),
            Some(FilteredContent::Text(text))
                if text == "SVG content was omitted because it could not be rendered safely."
        ));
    }
